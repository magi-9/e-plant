...
export default function ProductsPage() {
    ...
    const categories = Array.from(
        new Set((products || []).flatMap((product: Product) => getCategoryList(product)))
    );
    ...
    const filteredProducts = (products || []).filter((product: Product) => {
        if (searchQuery) {
            ...
            const categoriesMatch = getCategoryList(product).some((category) =>
                category.toLowerCase().includes(query)
            );
            if (!nameMatch && !descMatch && !categoriesMatch) return false;
        }
