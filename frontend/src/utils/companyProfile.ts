...
const FALLBACK_CONTACT_EMAIL = 'info@ebringer.sk';
...
export function getCompanyProfile(settings?: Partial<GlobalSettings>): CompanyProfile {
    ...
    const companyEmail = (settings?.company_email || '').trim() || FALLBACK_CONTACT_EMAIL;
    const warehouseEmail =
        (settings?.warehouse_email || '').trim() || FALLBACK_CONTACT_EMAIL;
