...
class VerifyEmailView(views.APIView):
    ...
    def post(self, request):
        ...
        if not uidb64 or not token:
            return Response({"error": "Chýbajúce údaje pre overenie."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if user is not None and default_token_generator.check_token(user, token):
            user.is_active = True
            user.save()
            return Response({"success": "E-mail bol úspešne overený. Teraz sa môžete prihlásiť."}, status=status.HTTP_200_OK)
        else:
            return Response({"error": "Overovací odkaz je neplatný alebo už expiroval."}, status=status.HTTP_400_BAD_REQUEST)
