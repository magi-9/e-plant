...
def test_import_products_create_and_update(api_client, user_factory):
    ...
    # Initial state
    Product.objects.create(name="Existing Product", price=10.00, stock_quantity=5, low_stock_threshold=5)

    # Create CSV
    output = StringIO()
    writer = csv.writer(output)
    writer.writerow(['name', 'description', 'category', 'price', 'stock_quantity', 'low_stock_threshold'])

    # Row 1: Update existing product
    writer.writerow(['Existing Product', 'Updated Description', 'Updated Category', '20.00', '50', '5'])

    # Row 2: Create new product
    writer.writerow(['New Product', 'New Description', 'New Category', '30.00', '100', '10'])

    csv_content = output.getvalue().encode('utf-8-sig')
    uploaded_file = SimpleUploadedFile("products.csv", csv_content, content_type="text/csv")
