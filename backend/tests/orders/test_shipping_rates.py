...
class TestShippingRateModel:
    def test_create_shipping_rate(self):
        ...
        rate = ShippingRate.objects.create(
            country="SK", carrier="SPS", price=Decimal("3.90"), free_above=Decimal("50.00")
        )
    ...
    def test_str_representation(self):
        from orders.models import ShippingRate
        rate = ShippingRate.objects.create(country="SK", carrier="SPS", price=Decimal("3.90"))
        assert "SK" in str(rate)
        assert "SPS" in str(rate)

    def test_free_above_nullable(self):
        from orders.models import ShippingRate
        rate = ShippingRate.objects.create(country="CZ", carrier="PPL", price=Decimal("5.50"))
