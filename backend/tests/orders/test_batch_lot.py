...
class TestBatchLotModel:
    def test_create_batch_lot(self):
        ...
        lot = BatchLot.objects.create(product=product, batch_number="BATCH-001", quantity=50)
    ...
    def test_batch_number_unique_per_product(self):
        ...
        with pytest.raises(IntegrityError):
            BatchLot.objects.create(product=product, batch_number="BATCH-001", quantity=5)
    ...
    def test_same_batch_number_different_products_allowed(self):
        ...
        lot2 = BatchLot.objects.create(product=p2, batch_number="BATCH-001", quantity=20)
    ...
    def test_ordered_by_received_at_asc(self):
        ...
        import datetime
        product = ProductFactory()
        BatchLot.objects.create(product=product, batch_number="FIRST", quantity=5)
        BatchLot.objects.create(product=product, batch_number="SECOND", quantity=5)
...
class TestStockReceiptService:
    ...
    def test_increments_existing_batch_lot(self):
        ...
        StockReceiptService.receive_stock(product=product, batch_number="B001", quantity=5)
        product.refresh_from_db()
    ...
    def test_creates_audit_record(self):
        ...
        from orders.services.stock_receipt_service import StockReceiptService
        product = ProductFactory(stock_quantity=0)
        StockReceiptService.receive_stock(product=product, batch_number="B001", quantity=10)
    ...
    def test_validates_quantity_positive(self):
        ...
        with pytest.raises(ValidationError):
            StockReceiptService.receive_stock(product=product, batch_number="B001", quantity=0)
    ...
    def test_validates_batch_number_not_empty(self):
        ...
        product = ProductFactory()
        with pytest.raises(ValidationError):
            StockReceiptService.receive_stock(product=product, batch_number="", quantity=5)
