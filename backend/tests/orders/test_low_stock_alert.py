...
def test_low_stock_alert_sent(api_client, user_factory, product_factory):
    ...
    product = product_factory(name="Test Product", price=Decimal("100.00"), stock_quantity=10, low_stock_threshold=5, low_stock_alert_sent=False)
    ...
    low_stock_email = [email for email in mail.outbox if "Nízky stav zásob" in email.subject]
...
def test_low_stock_alert_not_sent_again(api_client, user_factory, product_factory):
    ...
    product = product_factory(name="Test Product", price=Decimal("100.00"), stock_quantity=6, low_stock_threshold=10, low_stock_alert_sent=True)
...
def test_low_stock_alert_reset_on_replenish(product_factory):
    ...
    product = product_factory(name="Test Product", price=Decimal("100.00"), stock_quantity=4, low_stock_threshold=5, low_stock_alert_sent=True)
